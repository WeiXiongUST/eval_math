...
def load_and_merge_datasets(dataset_names: str):
    # 分割输入字符串，获取数据集名称列表
    dataset_list = dataset_names.split(',')
    
    # 加载所有数据集
    datasets = [load_dataset("weqweasdas/" + name.strip(), split="train") for name in dataset_list]

    merged_dataset = concatenate_datasets(datasets)
    
    return merged_dataset

ds = load_and_merge_datasets(script_args.dataset_name_or_path)
print(ds)
ds = ds.map(
    lambda x: {
...
one_num_share = int(data_size / script_args.my_world_size) + 1
ds = ds.select(np.arange(script_args.local_index * one_num_share, np.min([(script_args.local_index + 1) * one_num_share, data_size])) )
...
for i, output in enumerate(outputs):
    tmp_data = {"problem": ds[i]['problem'], "answers": [out.text for out in output.outputs], "gt":ds[i]['gt'], 'data_source': ds[i]['data_source']}
